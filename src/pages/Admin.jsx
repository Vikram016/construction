...
const Admin = () => {
  ...
  const handleToggle = async (p) => {
    try {
      ...
      notify(`Product ${p.isActive ? "disabled" : "enabled"} ✓`);
      await refetch();
    } catch (err) {
      notify("Error: " + err.message);
    }
  ...
  return (
    <div className="min-h-screen bg-neutral-50">
      ...
      <div className="max-w-7xl mx-auto px-4 py-6 space-y-6">
        ...
        {/* Search */}
        <input
          className="w-full border-2 border-neutral-200 rounded-lg px-4 py-2 text-sm focus:border-construction-yellow focus:outline-none"
                ...
                <thead>
                  <tr className="bg-neutral-50 border-b border-neutral-200 text-left">
                    {[
                      ...
                      <th
                        key={h}
                        className="px-4 py-3 font-bold text-neutral-700 whitespace-nowrap"
                      >
                        {h}
                      </th>
                    ))}
                      ...
                      >
                        <td className="px-4 py-3">
                          {p.images && p.images.length > 0 ? (
                            ...
                            <div className="w-12 h-12 bg-neutral-200 rounded-lg flex items-center justify-center text-xl">
                              📦
                            </div>
                          )}
                        </td>
                        ...
                        <td className="px-4 py-3">
                          ...
                          <p className="text-xs text-neutral-400">
                            ₹{p.basePrice} + {p.gstPercentage}% GST
                          </p>
                        </td>
                        <td className="px-4 py-3">
                          <span
                            className={`font-bold ${lowStock ? "text-red-600" : "text-green-600"}`}
                          >
                            {p.stock}
                          </span>
                          {lowStock && (
                        ...
                        <td className="px-4 py-3">
                          <div className="flex gap-2">
                            <button
                              ...
                              className="text-xs bg-blue-100 text-blue-700 hover:bg-blue-200 px-3 py-1.5 rounded-lg font-semibold"
                            >
                              Edit
                            </button>
                            <button
                              onClick={() => handleDelete(p)}
                              className="text-xs bg-red-100 text-red-700 hover:bg-red-200 px-3 py-1.5 rounded-lg font-semibold"
                            >
                              Delete
                            </button>
                          </div>
                    ...
                    <tr>
                      ...
                      >
                        ...
                        <button
                          ...
                          className="text-construction-yellow font-semibold underline"
                        >
                          Add one
                        </button>
                      </td>
