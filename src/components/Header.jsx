    ...
    <header className="bg-white shadow-sm border-b border-neutral-100 sticky top-0 z-50">
        ...
        <div className="container-custom flex items-center justify-between py-1.5">
          <div className="flex items-center gap-4">
            <span className="flex items-center gap-1.5">
              ...
              <span className="hidden sm:inline">Delivered 24–48 hrs · Bangalore</span>
              <span className="sm:hidden">24–48hr Delivery</span>
            </span>
            <span className="flex items-center gap-1.5">
              <svg className="w-3.5 h-3.5 text-green-400" fill="currentColor" viewBox="0 0 20 20" aria-hidden="true">
                <path fillRule="evenodd" d="M10 18a8 8 0 100-16 8 8 0 000 16zm3.707-9.293a1 1 0 00-1.414-1.414L9 10.586 7.707 9.293a1 1 0 00-1.414 1.414l2 2a1 1 0 001.414 0l4-4z" clipRule="evenodd"/>
          ...
          <a href={`tel:${WA}`} className="flex items-center gap-1.5 hover:text-orange-400 transition-colors">
      ...
      <div className="container-custom">
        <div className="flex items-center justify-between h-16 gap-4">
          ...
          {/* Logo */}
          <Link to="/" aria-label="BuildMart — Home" className="flex items-center gap-2.5 shrink-0">
            <div className="w-9 h-9 bg-orange-500 rounded-lg flex items-center justify-center shadow-sm">
              <span className="text-white font-black text-base leading-none select-none">BM</span>
            </div>
            <div className="hidden sm:block leading-none">
              <p className="font-display font-black text-lg text-neutral-900 leading-tight">BuildMart</p>
              <p className="text-[10px] text-neutral-400 font-medium tracking-wide">Premium Materials · Bangalore</p>
            </div>
          </Link>

          {/* ── Desktop nav ─────────────────────────────────────────────────── */}
          <nav className="hidden md:flex items-center gap-1 lg:gap-2" role="navigation" aria-label="Primary navigation">

            {/* Left plain links */}
            {NAV_LEFT.map((item) => (
              <Link key={item.path} to={item.path} className={`px-2.5 py-1 ${linkCls(isActive(item.path))}`}>
                {item.name}
              </Link>
            ))}
            ...
            >
              ...
              >
                {/* Arrow tip */}
                <div className="absolute -top-1.5 left-1/2 -translate-x-1/2 w-3 h-3 bg-white border-l border-t border-neutral-100 rotate-45" aria-hidden="true" />

                <div className="p-2 pt-3">
                  <p className="text-[10px] font-black uppercase tracking-widest text-neutral-400 px-3 pb-2">Our Services</p>
                  {SERVICES.map((svc) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-xl mt-0.5 shrink-0" aria-hidden="true">{svc.icon}</span>
                        <div className="min-w-0">
                          <p className={`font-bold text-sm leading-tight ${active ? 'text-orange-600' : 'text-neutral-900'}`}>
                            {svc.name}
                          </p>
                          <p className="text-xs text-neutral-400 mt-0.5 truncate">{svc.desc}</p>
                        </div>
                        {active && <div className="ml-auto mt-1.5 w-1.5 h-1.5 rounded-full bg-orange-500 shrink-0" aria-hidden="true" />}
                      </Link>
                    );
                  })}
                </div>
                ...
                <div className="bg-neutral-50 border-t border-neutral-100 px-4 py-2.5">
                  <p className="text-[11px] text-neutral-400">Serving Bangalore & surrounding areas</p>
                </div>
              </div>
            </div>
          ...
          <div className="flex items-center gap-1.5">
            ...
            <a
              ...
              className="hidden lg:inline-flex items-center gap-2 bg-[#25D366] hover:bg-[#1fba5a] text-white font-bold text-sm px-4 py-2 rounded-xl transition-colors"
            >
              <WAIcon />
              <span>Get Quote</span>
            </a>
        ...
        >
          <nav className="flex flex-col gap-0.5 py-3 border-t border-neutral-100">
            ...
            {NAV_LEFT.map((item) => (
              <Link
                ...
                className={`px-4 py-3 rounded-xl font-semibold text-sm transition-colors ${
                  isActive(item.path) ? 'bg-orange-50 text-orange-600' : 'text-neutral-700 hover:bg-neutral-100'
                }`}
              >
                {item.name}
              </Link>
            ...
            <div>
              <button
                ...
                className={`w-full flex items-center justify-between px-4 py-3 rounded-xl font-semibold text-sm transition-colors ${
                  isServiceActive ? 'bg-orange-50 text-orange-600' : 'text-neutral-700 hover:bg-neutral-100'
                }`}
              >
                <span>Services</span>
                ...
                <div className="ml-4 mt-1 mb-1 space-y-0.5 border-l-2 border-neutral-100 pl-3">
                  {SERVICES.map((svc) => {
                    ...
                    return (
                      ...
                      >
                        <span className="text-base mt-0.5 shrink-0" aria-hidden="true">{svc.icon}</span>
                        <div>
                          <p className="font-bold text-sm leading-tight">{svc.name}</p>
                          <p className="text-xs text-neutral-400 mt-0.5">{svc.desc}</p>
                        </div>
                      </Link>
                    );
                  })}
            ...
            <a
              ...
              target="_blank"
              rel="noopener noreferrer"
              className="mt-2 flex items-center justify-center gap-2 bg-[#25D366] text-white font-bold py-3 rounded-xl"
            >
              <WAIcon />
