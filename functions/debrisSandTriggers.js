...
exports.onDebrisSandBookingCreated = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      const result = await sendDebrisSandToSheet({
        ...bookingData,
        id: bookingId,
      ...
      console.log(
        `[DebrisSand] Booking ${bookingId} synced to Google Sheets:`,
        result.success,
      );
    } catch (error) {
...
exports.resendDebrisSandToSheet = functions.https.onCall(
  async (data, context) => {
    if (!context.auth) {
...
exports.retryFailedDebrisSandSyncs = functions.pubsub
  ...
  .onRun(async () => {
    ...
    try {
      ...
      console.log(
        `[DebrisSand] Retry complete: ${ok}/${results.length} succeeded`,
      );
