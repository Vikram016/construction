...
exports.onEstimateCreated = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      console.log(
        `Estimate ${estimateId} synced to Google Sheets:`,
        result.success,
      );
    } catch (error) {
      console.error(
        `Failed to sync estimate ${estimateId} to Google Sheets:`,
        error,
      );
...
exports.onInquiryCreated = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      console.log(
        `Inquiry ${inquiryId} synced to Google Sheets:`,
        result.success,
      );
    } catch (error) {
      console.error(
        `Failed to sync inquiry ${inquiryId} to Google Sheets:`,
        error,
      );
...
exports.resendInquiryToSheet = functions.https.onCall(async (data, context) => {
  ...
  try {
    ...
    const inquiryData = {
      ...
      id: inquiryId,
      type:
        collection === "estimates" ? "estimate" : doc.data().type || "inquiry",
    };
...
exports.retryFailedInquirySyncs = functions.pubsub
  ...
  .onRun(async (context) => {
    ...
    try {
      ...
      const failedEstimates = await admin
        .firestore()
      ...
      const failedInquiries = await admin
        .firestore()
      ...
      console.log(
        `Found ${failedEstimates.size} failed estimates and ${failedInquiries.size} failed inquiries to retry`,
      );
      ...
      console.log(
        `Retry complete: ${successCount}/${results.length} successful`,
      );
