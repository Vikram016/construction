...
const formatDate = (date) => {
  const d = date && date.toDate ? date.toDate() : date ? new Date(date) : new Date();
...
const createInvoicePDF = (orderData, invoiceNumber) =>
  new Promise((resolve, reject) => {
    try {
      ...
      // Brand
      doc.font("Helvetica-Bold").fontSize(26).fillColor(YELLOW).text("BUILDMART", 45, 25);
      doc.font("Helvetica").fontSize(9).fillColor(GREY).text(COMPANY.tagline, 45, 55);
      ...
      doc.font("Helvetica").fontSize(8).fillColor(BLACK)
         ...
         .font("Helvetica-Bold")
         .text("GSTIN: " + COMPANY.gstin, 350, 82, { align: "right", width: 200 });

      doc.moveTo(45, 100).lineTo(550, 100).strokeColor("#E5E7EB").lineWidth(1).stroke();

      // Invoice title
      doc.font("Helvetica-Bold").fontSize(18).fillColor(BLACK).text("INVOICE", 45, 115);

      // Meta box
      doc.rect(45, 140, W, 70).strokeColor(YELLOW).lineWidth(1.5).stroke();
      const col2 = 300;
      doc.font("Helvetica-Bold").fontSize(9).fillColor(GREY)
         .text("INVOICE NO.",   55, 153).text("DATE",           55, 170)
         .text("ORDER ID",      55, 187).text("PAYMENT STATUS", col2, 153)
         .text("PAYMENT MODE",  col2, 170);

      const payStatus = orderData.payment && orderData.payment.status || "Pending";
      const payColor  = payStatus === "Paid" ? GREEN : payStatus === "Pending" ? "#D97706" : RED;

      doc.font("Helvetica").fontSize(9).fillColor(BLACK)
         .text(invoiceNumber, 150, 153)
         .text(formatDate(orderData.createdAt), 150, 170)
         .text(orderData.orderNumber || (orderData.id || "").slice(0, 8).toUpperCase() || "N/A", 150, 187);

      doc.font("Helvetica-Bold").fillColor(payColor)
         .text(payStatus, col2 + 95, 153);
      doc.font("Helvetica").fillColor(BLACK)
         .text(orderData.paymentMethod || (orderData.payment && orderData.payment.method) || "WhatsApp Order", col2 + 95, 170);
      ...
      doc.font("Helvetica-Bold").fontSize(9).fillColor(GREY).text("BILL TO / SHIP TO", 45, 228);
      doc.font("Helvetica-Bold").fontSize(10).fillColor(BLACK).text(c.name || "Customer", 45, 243);
      ...
      items.forEach((item, i) => {
        ...
        const total = ((item.basePrice || 0) * (item.quantity || 1)).toLocaleString("en-IN");
        doc.font("Helvetica").fontSize(8).fillColor(BLACK)
        ...
        y += rowH;
        if (y > 680 && i < items.length - 1) { doc.addPage(); y = 45; }
      });

      doc.moveTo(45, y).lineTo(550, y).strokeColor("#E5E7EB").lineWidth(1).stroke();
      y += 10;
      ...
      const addRow = (label, val, bold, color) => {
        doc.font(bold ? "Helvetica-Bold" : "Helvetica").fontSize(9)
           .fillColor(GREY).text(label, 360, y, { width: 100 })
           .fillColor(color || BLACK).text("Rs." + val.toLocaleString("en-IN"), 465, y, { width: 80, align: "right" });
        y += 16;
      };
      addRow("Subtotal", pricing.subtotal || 0);
      if (pricing.deliveryCharge) addRow("Delivery Charge", pricing.deliveryCharge);
      ...
      doc.font("Helvetica-Bold").fontSize(11).fillColor(YELLOW)
         .text("GRAND TOTAL", 360, y + 8, { width: 100 })
         .text("Rs." + (pricing.grandTotal || 0).toLocaleString("en-IN"), 460, y + 8, { width: 85, align: "right" });
      y += 38;
      // Payment status + ID
      const payStatusLabel = orderData.payment && orderData.payment.status || "Pending";
      const payIdLabel     = payId ? payId : "N/A";
      const payColor2      = payStatusLabel === "Paid" ? GREEN : RED;
      doc.font("Helvetica-Bold").fontSize(8).fillColor(GREY).text("Payment Status:", 360, y, { width: 100 });
      doc.font("Helvetica-Bold").fontSize(8).fillColor(payColor2).text(payStatusLabel, 465, y, { width: 80, align: "right" });
      y += 14;
      doc.font("Helvetica-Bold").fontSize(8).fillColor(GREY).text("Payment ID:", 360, y, { width: 100 });
      doc.font("Helvetica").fontSize(8).fillColor(BLACK).text(payIdLabel, 465, y, { width: 80, align: "right" });
      y += 18;

      // Notes
      if (c.notes) {
        doc.font("Helvetica-Bold").fontSize(8).fillColor(GREY).text("DELIVERY NOTES", 45, y);
        doc.font("Helvetica").fontSize(8).fillColor(BLACK).text(c.notes, 45, y + 12, { width: 300 });
        y += 36;
      }

      // Terms
      if (y > 700) { doc.addPage(); y = 45; }
      doc.font("Helvetica-Bold").fontSize(8).fillColor(GREY).text("TERMS & CONDITIONS", 45, y + 10);
      doc.font("Helvetica").fontSize(7.5).fillColor(GREY)
         .text("1. Goods once delivered cannot be returned unless damaged in transit.", 45, y + 24)
      ...
      doc.font("Helvetica").fontSize(7.5).fillColor(GREY)
         .text("This is a computer-generated invoice and does not require a physical signature.", 45, 793, { align: "center", width: W })
         .text(COMPANY.website + "  .  " + COMPANY.email + "  .  " + COMPANY.phone, 45, 806, { align: "center", width: W });
...
const sendInvoiceToCustomer = async ({ orderData, invoiceNumber, invoiceUrl, pdfBuffer }) => {
  ...
  try {
    ...
    const itemRows = items.map(item =>
      "<tr><td style=\"padding:8px 12px;border-bottom:1px solid #f3f4f6;\">" + (item.name || item.productName) + "</td>" +
      "<td style=\"padding:8px 12px;text-align:center;border-bottom:1px solid #f3f4f6;\">" + item.quantity + " " + (item.unit || "") + "</td>" +
      "<td style=\"padding:8px 12px;text-align:right;border-bottom:1px solid #f3f4f6;\">Rs." + (item.basePrice || 0).toLocaleString("en-IN") + "</td>" +
      "<td style=\"padding:8px 12px;text-align:right;font-weight:600;border-bottom:1px solid #f3f4f6;\">Rs." + ((item.basePrice || 0) * (item.quantity || 1)).toLocaleString("en-IN") + "</td></tr>"
    ).join("");

    const htmlBody = "<!DOCTYPE html><html><body style=\"margin:0;background:#f9fafb;font-family:Arial,sans-serif;\">" +
      ...
      "<tr><td style=\"padding:32px;\">" +
        "<p style=\"color:#374151;margin:0 0 6px;\">Hello <strong>" + (customer.name || "Valued Customer") + "</strong>,</p>" +
        ...
        "<table width=\"100%\" style=\"background:#fffbeb;border:1px solid #fde68a;border-radius:6px;margin-bottom:24px;\">" +
          "<tr><td style=\"padding:16px 20px;\">" +
            ...
            "<p style=\"margin:4px 0 0;font-size:20px;font-weight:900;font-family:monospace;\">" + invoiceNumber + "</p>" +
          "</td><td style=\"padding:16px 20px;text-align:right;\">" +
            "<a href=\"" + invoiceUrl + "\" style=\"display:inline-block;background:#FDB913;color:#1a1a1a;font-weight:700;font-size:13px;padding:10px 20px;border-radius:4px;text-decoration:none;\">Download PDF</a>" +
          "</td></tr>" +
        "</table>" +
        "<table width=\"100%\" style=\"border:1px solid #f3f4f6;border-radius:6px;margin-bottom:20px;font-size:13px;\">" +
          ...
          "<tfoot>" +
            "<tr><td colspan=\"3\" style=\"padding:10px 12px;text-align:right;color:#6b7280;font-size:12px;\">Subtotal</td><td style=\"padding:10px 12px;text-align:right;\">Rs." + (pricing.subtotal || 0).toLocaleString("en-IN") + "</td></tr>" +
            // GST row removed —
            (pricing.deliveryCharge ? "<tr><td colspan=\"3\" style=\"padding:4px 12px;text-align:right;color:#6b7280;font-size:12px;\">Delivery</td><td style=\"padding:4px 12px;text-align:right;\">Rs." + pricing.deliveryCharge.toLocaleString("en-IN") + "</td></tr>" : "") +
            "<tr style=\"background:#1a1a1a;\">" +
              "<td colspan=\"3\" style=\"padding:12px;text-align:right;color:#FDB913;font-weight:700;\">GRAND TOTAL</td>" +
              "<td style=\"padding:12px;text-align:right;color:#FDB913;font-weight:900;font-size:15px;\">Rs." + (pricing.grandTotal || 0).toLocaleString("en-IN") + "</td>" +
            "</tr>" +
          "</tfoot>" +
        "</table>" +
    ...
    const rawEmail = [
      ...
      "To: " + (customer.name ? "\"" + customer.name + "\" <" + toEmail + ">" : toEmail),
      "Subject: " + subject,
      "MIME-Version: 1.0",
