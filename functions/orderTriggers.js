...
exports.onOrderCreated = functions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    console.log(`[orderTriggers] New order: ${orderId} | Payment: ${orderData.payment?.status}`);

    if (!isPaid(orderData.payment?.status)) {
      console.log(`[orderTriggers] ${orderId} not yet paid — skipping sheets sync`);
      return null;
    }

    const paymentId = orderData.payment?.paymentId || 'N/A';
    const result    = await syncToSheets(snap.ref, orderData, paymentId);
    console.log(`[orderTriggers] onCreate sheets sync ${orderId}: ${result.success}`);
    return null;
  });
...
exports.onOrderPaymentConfirmed = functions.firestore
  ...
  .onUpdate(async (change, context) => {
    ...
    console.log(`[orderTriggers] onUpdate sheets sync ${orderId}: ${result.success}`);
    return null;
  });
...
exports.resendOrderToSheet = functions.https.onCall(async (data, context) => {
  ...
  return {
    success:  result.success,
    orderId,
    message:  result.success ? 'Synced to Google Sheets' : 'Sync failed: ' + result.error,
  };
});
