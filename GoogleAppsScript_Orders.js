/**
 * AnjaneyaDealers — Google Apps Script
...
function doGet(e) {
  ...
  if (action === "setup") {
    ...
    return ContentService.createTextOutput(
      "Setup complete. Sheets created.",
    ).setMimeType(ContentService.MimeType.TEXT);
  }
  return ContentService.createTextOutput(
    "AnjaneyaDealers Order Webhook — OK",
  ).setMimeType(ContentService.MimeType.TEXT);
}
...
function handleOrder(data) {
  ...
  const itemsSummary = items
    .map(
      (it) =>
        `${it.name} ×${it.quantity} ${it.unit} = ₹${(it.subtotal || 0).toLocaleString("en-IN")}`,
    )
    .join("\n");
  ...
  return buildResponse(
    true,
    "Order " + orderId + " saved. Items: " + items.length,
  );
}
...
function buildResponse(success, message) {
  return ContentService.createTextOutput(
    JSON.stringify({ success, message, ts: new Date().toISOString() }),
